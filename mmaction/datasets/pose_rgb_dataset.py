...
class RGBPoseDataset(BaseDataset):
    ...
    def load_pkl_annotations(self):
        ...
        for item in data:
            ...
            # adaption for gym 
            file_name = item['frame_dir'].split("_")
            file_name.insert(1, 'E')
            file_name.insert(4, 'A')
            file_name = '_'.join(file_name) + '.mp4'
            item['filename'] = osp.join(self.video_path, file_name )
            
            if osp.exists(item['filename']):
    ...
    def load_video_annotations(self):
        """Load vide file to get video information."""

        video_infos = []
        with open(self.video_path, 'r') as fin:
            for line in fin:
